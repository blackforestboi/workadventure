...
export class FloorEditorTool extends MapEditorTool {
    ...
    private paintWaterTile(
        ...
        const coverFamilies = getTerrainAutotileFamilies(visibleMap);
        ...
        const contourReferenceLayer = findWaterContourReferenceLayer(
            visibleMap,
            coverLayer,
            previous,
            tile,
        ...
        const contourReference =
            contourReferenceLayer === undefined
                ? undefined
                : createWaterTerrainBrushRegions(
                      contourReferenceLayer.name,
                      waterLayerName,
                      previous,
                      tile,
                      waterGid,
                      collectTerrainTiles(contourReferenceLayer, previous, tile, 3),
                      waterLayer?.type === "tilelayer" ? collectTerrainTiles(waterLayer, previous, tile, 3) : [],
                      coverFamilies,
                  );
        if (contourReference !== undefined) {
            const referenceLayerName = contourReferenceLayer?.name;
            composition.regions.push(
                ...contourReference.regions
                    .filter((region) => region.layer === referenceLayerName && region.gids.some((gid) => gid !== 0))
                    .map((region) => ({ ...region, layer: waterLayerName })),
            );
        }
    ...
    private finishShapeDrag(): void {
        ...
        if (brush.kind === "water" && terrainLayer?.type === "tilelayer") {
            ...
            const coverFamilies = getTerrainAutotileFamilies(source);
            ...
            if (contourReferenceLayer !== undefined) {
                const contourReference = createWaterTerrainRectangleRegions(
                    contourReferenceLayer.name,
                    waterLayerName,
                    start,
                    end,
                    brush.gid,
                    collectTerrainTiles(contourReferenceLayer, start, end, 2),
                    waterLayer?.type === "tilelayer" ? collectTerrainTiles(waterLayer, start, end, 2) : [],
                    coverFamilies,
                );
                composition.regions.push(
                    ...contourReference.regions
                        .filter(
                            (region) =>
                                region.layer === contourReferenceLayer.name && region.gids.some((gid) => gid !== 0),
                        )
                        .map((region) => ({ ...region, layer: waterLayerName })),
                );
            }
...
function findWaterContourReferenceLayer(
    map: ITiledMap,
    coverLayer: ITiledMapTileLayer,
    start: { x: number; y: number },
    end: { x: number; y: number },
    padding: number,
    families: readonly TerrainAutotileFamily[],
): ITiledMapTileLayer | undefined {
    if (families.length === 0) return undefined;
    const hasFamilyTiles = (layer: ITiledMapTileLayer): boolean =>
        collectTerrainTiles(layer, start, end, padding).some(({ gid }) =>
            families.some((family) => family.gids.has(gid)),
        );

    // Prefer the selected cover's own family whenever it is available.
    if (hasFamilyTiles(coverLayer)) return undefined;
    const baseName = surfaceOverlayCoverLayerName(coverLayer.name);
    if (baseName === undefined) return undefined;

    const flattened = flattenLayers(map.layers);
    const coverIndex = flattened.findIndex((layer) => layer.name === coverLayer.name);
    for (let index = coverIndex - 1; index >= 0; index -= 1) {
        const candidate = flattened[index];
        if (
            candidate.type !== "tilelayer" ||
            candidate.visible === false ||
            (candidate.name !== baseName && surfaceOverlayCoverLayerName(candidate.name) !== baseName)
        ) {
            continue;
        }
        if (hasFamilyTiles(candidate)) return candidate;
    }
    return undefined;
}
