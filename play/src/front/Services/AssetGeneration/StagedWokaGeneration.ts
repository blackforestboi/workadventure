...
function idleFrameRules(target: WokaGenerationTarget, usesCompositionReference: boolean): string {
    ...
    return [
        ...
        `Generate exactly one detailed square design of ${subject}. This is the fixed Woka front-idle pose: a direct 0-degree front elevation facing straight into the camera (the ${BRANDING.name} down direction). The camera is perpendicular to the chest and face: 0-degree yaw, 0-degree pitch, and 0-degree roll. Never make a side, three-quarter, back, walking, leaning, seated, action, or head-turned pose.`,
